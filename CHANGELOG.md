...
- **README + CONTRIBUTING audit pass.** Closed nine stale / inconsistent / misleading claims accumulated since v0.1: (1) Python version range corrected from "3.11 or 3.12" to "3.11 through 3.14" (matches the v0.2 pin bump). (2) plink2 install snippet now offers Linux x86_64, macOS ARM64, and macOS Intel asset names instead of Linux-only. (3) `--target` option now documents repeatability + multi-target suffix scheme (`_target_<input_idx>` for ≥ 2 targets). (4) NFS/SMB/CIFS detection scope clarified as Linux-only (matches `concurrency.detect_network_filesystem`). (5) Performance numbers updated from v0.1's 25-30 M g/s claim to v0.3.1's measured 70.84 M g/s baseline. (6) `.geno_prefix` invented file extension replaced with real EIGENSTRAT prefix usage in canonical-use-case 3 and the `hash` example. (7) Subcommand synopsis alignment normalized. (8) Validation-gate section rewritten to clarify that gates (a)-(c) apply to `merge` and all four (a)-(d) apply to `validate`. (9) Dogfood tier-running commands in CONTRIBUTING corrected — the in-test `pytest.skipif(HAS_PLINK2)` / `HAS_ADMIXTOOLS` probes gate tier-2/3 execution, not the marker selector; bare `pytest tests/dogfood/` runs whatever your machine supports. (10) HLD-test-N references replaced with concrete test-file paths. (11) `mypy` scope made explicit (configured `files = ["src/pgen_samplebind"]`). (12) Perf-baseline calibration procedure gained a concrete v0.3.1 example.
