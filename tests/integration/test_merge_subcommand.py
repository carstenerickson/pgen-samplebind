...
Minimal coverage: smoke test (two panels with shared variants but
disjoint samples concatenate correctly), --on-collision first behavior,
.psam output structure.
"""
...
from tests.fixtures.helpers import read_pgen_full, read_psam_iids, read_pvar_keys
...
class TestMultiInputMerge:
    ...
    def test_three_panels_concatenate(
        ...
        keys = read_pvar_keys(out)
        assert len(keys) == 100

        # Pgen has the right shape
        buf = read_pgen_full(out, n_samples=30, n_variants=100)
...
class TestMergeSmoke:
    def test_two_disjoint_panels_concatenate(
        ...
        for ext in (".pgen", ".pvar", ".psam"):
            assert Path(str(out) + ext).exists()
        ...
        # Pgen has the right shape
        buf = read_pgen_full(out, n_samples=20, n_variants=100)
